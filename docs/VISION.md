...
### 정체성 불변식 (최상위 — 재론은 비전 차원 재정의에서만; 권위: `ADR-20260712-palimpsest-identity-host-neutral-generative-curator`)

아래 두 축은 palimpsest의 **최상위 목표이자 정체성 불변식**이다. 아래 "세 목표"(품질 목표)보다 상위이며, 구현·소비자 구성의 변화로는 흔들지 않는다. 강한 진술이 비권위 문서(이 VISION·DESIGN)에만 있어 이미 두 번 드리프트가 났기에(provider-free 역전, ditto-결합 추론), 권위 ADR로 승격하고 always-loaded 컨텍스트로 상시 투영한다.

- **A. 환경 비종속 (host-neutral).** palimpsest는 어떤 소비 환경에도 종속되지 않는다. ditto는 **주 소비자이자 첫 소비자일 뿐**, 특정 호스트가 아니다. ⑴ 소비자-중립 표면(MCP/CLI)으로 일반 어휘를 노출하고 특정 소비자에 특화하지 않는다, ⑵ 소비자 도메인 개념(예: ditto ACG의 ImpactGraph 등)을 코어에 내부화하지 않는다(매핑은 소비자 쪽), ⑶ 코어가 특정 호스트 산출물에 의존하는 역방향 종속을 만들지 않는다(외부 producer는 producer-중립 `load`로 수용).
- **B. 생성형 큐레이터 (핵심 목적).** 단순 아카이브/사서가 아니라 **조합형 + 생성형 큐레이션으로 새 가치를 합성**하는 것이 핵심 목적이다. 생성형 출력은 출처 + gap + confidence로 사실과 분리(세탁 금지).
