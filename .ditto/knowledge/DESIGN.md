...
> 🔶 **아직 미실현:** inferred 엣지 `CAUSALLY_RELATES`·`RELATES_TO`(+`CONFLICTS_WITH`), 내용(semantic) 검증. *(`Summary`·`CommunityReport`·`Risk`(+`RISKS`)·`DesignDecision`(+`DECIDES`/`SUPERSEDES`/`ADDRESSES_RISK`) 로더는 실현 — ADR-20260701/20260702-*-load-contract, 위/아래 참조.)* (→ §6 로드맵)
