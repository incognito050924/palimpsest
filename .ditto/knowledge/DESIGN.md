...
> 🔶 **아직 미실현:** `DesignDecision/ADR` 노드(계약 초안됨·로더 유예, ADR-20260702-risk-designdecision-load-contract) 및 그 엣지(`DECIDES`·`SUPERSEDES`·`ADDRESSES_RISK`)·`CAUSALLY_RELATES`·`RELATES_TO`, 내용(semantic) 검증. *(`Risk`/`RISKS` 로더·`CommunityReport` 로더는 실현 — 위/아래 참조.)* (→ §6 로드맵)
...
- 🔶 **노드 — 정적층(CPG):** `Repo` · `Module/File` · `Type/Class` · `Method/Function` · `Variable/Local` · `CallSite` · `Community`(구조 그룹핑 ✅ 결정론). **의미층:** `Summary`(✅ slice 4)/`CommunityReport`(요약 prose ✅ 적재 실현·생성 외부) · `DesignDecision/ADR`(🔶 계약 초안) · `Risk/Finding`(✅ 로더 실현·생성 외부) · `Episode/SourceCommit`(✅).
- 🔶 **엣지 — 결정론적 구조(`edge_kind=deterministic`, ✅ v1):** `CONTAINS` · `CALLS` · `IMPORTS` · `DEPENDS_ON` · `MEMBER_OF`(✅ Class→Community) · (제안) `REACHING_DEF`(DATAFLOW) · `INHERITS` · `REF`. **생성형 추론(`edge_kind=inferred`):** `SUMMARIZES`(✅ slice 4) · `RISKS`(✅ Risk→코드, wi_2607021h0) · `CAUSALLY_RELATES`(🔶) · `ADDRESSES_RISK`(🔶) · `DECIDES`(🔶) · `RELATES_TO`(🔶).
...
- 🔶 **유예된 의미층 후속** — 실적재 경로로 실제 사용·고통을 확인하고 각 ADR change-condition을 재검토한 뒤 착수:
  ...
  | **#3 요약 대상 확장** | `Risk`/`DesignDecision` 노드로 대상 확대(`ADR-20260702-risk-designdecision-load-contract`). **`Risk` 로더 실현**(active, wi_2607021h0 — Risk 노드+RISKS inferred, grounded·entity-atomic; 위 roadmap). **`DesignDecision`**(+`DECIDES`/`SUPERSEDES`/`ADDRESSES_RISK`)은 유예. 외부 생산자(실데이터)는 provider-free상 밖. *(`Community` 구조 실현; `CommunityReport` 로더 실현 — `ADR-20260702-communityreport-load-contract` active, 생성만 외부.)* |
...
- ✅ `ADR-20260702-risk-designdecision-load-contract` — Risk·DesignDecision 적재 계약: `ADR-20260701`을 1급 inferred 엔티티로 일반화(namespace `risk:`/`decision:` id, inferred 엣지, grounded 노드 강제, 전용 로더). **Risk 부분 실현**(active, wi_2607021h0 — `Risk`+`RISKS`, `kg/risk.py`, 72 passed). **DesignDecision**(+`DECIDES`/`SUPERSEDES`/`ADDRESSES_RISK`)은 유예. → [`adr/ADR-20260702-risk-designdecision-load-contract.md`](adr/ADR-20260702-risk-designdecision-load-contract.md)
